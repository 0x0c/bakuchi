...
バケッティングの規範仕様は [spec/bucketing-ja.md](../spec/bucketing-ja.md)、検証は [spec/golden-vectors.json](../spec/golden-vectors.json)。
...
- 複数端末で一貫させたい場合は端末保存では足りず、`assignment-service` のサーバ側スティッキーストアを使う（[05 章](05-services-ja.md)）。
...
| A/A テスト | 本番で常時 2 本の A/A 実験を走らせ、偽陽性率が名目水準に収まることを監視（[07 章](07-statistics-ja.md)） |
