...
| [assignment-service](#8-assignment-servicephase-2) | データ | Go | Redis + DynamoDB | 2 |
| [guardrail-watcher](#9-guardrail-watcherphase-2) | 解析 | Go | ClickHouse | 2 |

Phase 1 では experiment / metric / builder / console-BFF を**単一デプロイ単位**にまとめる（[02 章 §4](02-architecture-ja.md#4-サービス分割の方針)）。以下は論理的な責務分割として読む。
...
| 時刻補正 | バッチの `sent_at` と `server_ts` の差から skew を推定し `corrected_ts` を付与（[06 章](06-data-pipeline-ja.md)） |
...
計算内容は [07 章](07-statistics-ja.md)。実装上の要点:
...
| 結果 | 指標ごとの効果量・信頼区間・逐次検定の判定。SRM 検知時は**結果を隠す**（[07 章](07-statistics-ja.md)） |
