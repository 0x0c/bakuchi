...
**最も効く削減策はイベントの間引き。** `reason != ASSIGNED` の曝露を 1% サンプリングする（[04 章 §6](04-client-sdk-ja.md#6-曝露exposureイベント)）、デバッグイベントを本番で送らない、といった設計時の判断が運用コストを一桁変える。
