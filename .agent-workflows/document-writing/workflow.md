...
bakuchi's writing is argued prose: roadmap items and the documents under `docs/` make a case a reader
...
- **Scope.** Every roadmap item (`roadmaps/BK-*/*.md` and its `*-ja.md` mirror) and every
  document under `docs/`, in both languages. It does **not** govern the normative specifications
  under `spec/`, which state requirements an implementation must satisfy rather than argue a case,
  and so are written as rules and tables instead of prose.
...
- **Enforcement stays a review-time norm, not a CI gate.** Judging clarity, emphasis, and argument
  order needs semantic judgment, which no deterministic check can supply. The mechanical floor that
  *is* gated covers the file's shape (`tools/check_roadmap_format.py`) and its typography
  (textlint); the argument itself is a contributor and reviewer expectation, checked by people.
...
- [`japanese-document-writing`](../japanese-document-writing/workflow.md) — the Japanese layer beneath this
  norm.
- [`roadmaps/README.md`](../../roadmaps/README.md) — the roadmap-item format this norm shapes the
  prose inside.
- The [Bajutsu](https://github.com/bajutsu-e2e/bajutsu) project — the origin of this norm, adapted
  here under the Apache License 2.0 (see [`../NOTICE`](../NOTICE)).
