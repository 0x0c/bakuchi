...
[`implement-bk`](../implement-bk/workflow.md). That workflow can hand these steps to a fresh
...
- Run `tools/check.sh` locally to verify.
- Push the fix.
...
- For each comment:
  ...
  3. Run `tools/check.sh` to verify.
...
unresolved for a self-review-only finding. Run `tools/check.sh` after every fix, the same as steps 2
and 3.
