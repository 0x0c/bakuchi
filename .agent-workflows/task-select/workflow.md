...
1. **Gather context**
   ...
   - Fetch open GitHub Issues:
     ...
     ```
   - Check the roadmap for Proposal / Accepted items:
     use the `/roadmap-filter` skill, or run `python3 tools/roadmap_query.py --status "Proposal"`.
...
5. **Wait for the user's choice** before suggesting next steps. When the user
   ...
   - `implement-bk BK-NNNN` to start implementation
